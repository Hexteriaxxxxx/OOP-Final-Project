...
public class StaffVisitorController implements Initializable {
    ...
    private void setupColumns() {
        ...
        colStatus.setCellFactory(col->new TableCell<>(){
            @Override protected void updateItem(String val,boolean empty){super.updateItem(val,empty);if(empty||val==null){setGraphic(null);return;}Label lbl=new Label(val);
            ...
            else lbl.setStyle("-fx-text-fill:#E6A817;-fx-font-weight:bold;");setGraphic(lbl);setText(null);}
        });
    }
    ...
    private void applyFilters(){String kw=txtSearch.getText().toLowerCase().trim();String status=cmbFilter.getValue();filteredList.setPredicate(v->{boolean matchSt="All".equals(status)||v.getStatus().equalsIgnoreCase(status);boolean matchKw=kw.isEmpty()||v.getVisitorName().toLowerCase().contains(kw)||v.getCompany().toLowerCase().contains(kw)||v.getPurpose().toLowerCase().contains(kw)||v.getRequestId().toLowerCase().contains(kw);return matchSt&&matchKw;});}
    ...
    @FXML private void handleNavPassSlip(){goTo("/main/resources/fxml/StaffPassSlipIssuance.fxml","Pass Slip");}
