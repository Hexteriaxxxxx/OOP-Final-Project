...
public class StaffPassSlipController implements Initializable {
    ...
    private void setupActionColumn() {
        colActions.setCellFactory(col->new TableCell<>(){
            ...
            {box.setAlignment(Pos.CENTER);btnDownload.setStyle("-fx-background-color:#8B0000;-fx-text-fill:white;-fx-font-size:11px;-fx-padding:5 10;-fx-cursor:hand;-fx-background-radius:5;");btnPrint.setStyle("-fx-background-color:transparent;-fx-text-fill:#E67E22;-fx-font-size:15px;-fx-cursor:hand;-fx-padding:2 5;");btnView.setStyle("-fx-background-color:transparent;-fx-text-fill:#1565C0;-fx-font-size:15px;-fx-cursor:hand;-fx-padding:2 5;");
            btnDownload.setOnAction(e->handleDownload(getTableView().getItems().get(getIndex())));btnPrint.setOnAction(e->handlePrint(getTableView().getItems().get(getIndex())));btnView.setOnAction(e->showDetails(getTableView().getItems().get(getIndex())));}
    ...
    private void refreshStats() {
        ...
        lblTotalApproved.setText(String.valueOf(total));lblTodayApproved.setText(String.valueOf(todayCount));lblDownloaded.setText(String.valueOf(downloadCount));lblPrinted.setText(String.valueOf(printCount));lblApprovedCount.setText(total+" approved requests");
    }

    @FXML private void handleSearch(){applyFilters();}
    @FXML private void handleFilter(){applyFilters();}
    private void applyFilters(){String kw=txtSearch.getText().toLowerCase().trim();String dept=cmbFilter.getValue();filteredList.setPredicate(ps->{boolean matchDept="All Departments".equals(dept)||ps.getDepartment().equalsIgnoreCase(dept);boolean matchKw=kw.isEmpty()||ps.getEmpName().toLowerCase().contains(kw)||ps.getDepartment().toLowerCase().contains(kw)||ps.getReason().toLowerCase().contains(kw)||String.valueOf(ps.getSlipId()).contains(kw);return matchDept&&matchKw;});}

    @FXML private void handleDownloadAll(){if(filteredList.isEmpty()){showError("No records.");return;}FileChooser fc=new FileChooser();fc.setInitialFileName("PassSlips_All.csv");fc.getExtensionFilters().add(new FileChooser.ExtensionFilter("CSV","*.csv"));File f=fc.showSaveDialog(tblSlips.getScene().getWindow());if(f!=null){try(FileWriter fw=new FileWriter(f)){fw.write("Slip ID,Name,Department,Purpose,Time Out,Time In,Duration,Status\n");for(PassSlip ps:filteredList)fw.write(String.format("PS-%04d,%s,%s,%s,%s,%s,%s,%s\n",ps.getSlipId(),ps.getEmpName(),ps.getDepartment(),ps.getReason(),ps.getFormattedTimeOut(),ps.getFormattedTimeIn(),ps.getDuration()!=null?ps.getDuration():"",ps.getStatus()));downloadCount+=filteredList.size();refreshStats();showInfo("Downloaded "+filteredList.size()+" record(s).");}catch(IOException e){showError("Failed: "+e.getMessage());}}}
    private void handleDownload(PassSlip ps){FileChooser fc=new FileChooser();fc.setInitialFileName("PassSlip_"+ps.getSlipId()+".csv");fc.getExtensionFilters().add(new FileChooser.ExtensionFilter("CSV","*.csv"));File f=fc.showSaveDialog(tblSlips.getScene().getWindow());if(f!=null){try(FileWriter fw=new FileWriter(f)){fw.write("Slip ID,Name,Department,Purpose,Time Out,Time In,Duration,Status\n");fw.write(String.format("PS-%04d,%s,%s,%s,%s,%s,%s,%s\n",ps.getSlipId(),ps.getEmpName(),ps.getDepartment(),ps.getReason(),ps.getFormattedTimeOut(),ps.getFormattedTimeIn(),ps.getDuration()!=null?ps.getDuration():"",ps.getStatus()));downloadCount++;refreshStats();showInfo("Downloaded: "+f.getPath());}catch(IOException e){showError("Failed: "+e.getMessage());}}}
    private void handlePrint(PassSlip ps){Label lbl=new Label(buildText(ps));lbl.setStyle("-fx-font-family:'Courier New';-fx-font-size:13px;-fx-padding:20;");PrinterJob job=PrinterJob.createPrinterJob();if(job!=null&&job.showPrintDialog(tblSlips.getScene().getWindow())){if(job.printPage(lbl)){job.endJob();printCount++;refreshStats();showInfo("Sent to printer.");}else showError("Printing failed.");}else if(job==null)showError("No printer found.");}
