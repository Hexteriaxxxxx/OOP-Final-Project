...
public class NotificationHelper {
    ...
    public void toggle() {
        ...
        if (popup != null && popup.isShowing()) {
            popup.hide(); bellBtn.setStyle(BELL_NORMAL); return;
        }
        ...
        container.setStyle("-fx-background-color: white; -fx-background-radius: 12; -fx-border-radius: 12; -fx-border-color: #e8e8e8; -fx-border-width: 1; -fx-effect: dropshadow(gaussian, rgba(0,0,0,0.18), 16, 0, 0, 4);");
    ...
    private HBox buildRow(NotifItem notif) {
        ...
        lblText.setWrapText(true); lblText.setMaxWidth(200);
    ...
    private void refreshNotifications() {
        ...
        try {
            ...
            List<models.PassSlip> all = passSlipDAO.getAllPassSlips();
            ...
            if (role == Role.ADMIN) {
                ...
                long approved = today.stream().filter(p -> "Approved".equalsIgnoreCase(p.getStatus())).count();
                int  active   = passSlipDAO.countActiveSlips();
                if (approved > 0)
                    notifications.add(new NotifItem("✅",
