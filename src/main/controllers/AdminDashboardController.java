...
public class AdminDashboardController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        // ── Show skeleton, load data in background ──────────────
    ...
    /** Background data load — hides skeleton when done */
    ...
    private void updateStatCards(List<PassSlip> all) {
        ...
        } catch (Exception ex) { System.out.println("Today stats error: " + ex.getMessage()); }
    }
    ...
    /** Reload with skeleton — use after actions like approve/reject/return */
    ...
    private void handleApprovePassSlip(PassSlip ps) {
        ...
        confirm.setTitle("Approve Pass Slip"); confirm.setHeaderText("Approve request #" + ps.getSlipId() + "?");
        ...
        confirm.showAndWait().ifPresent(r -> {
            if (r == ButtonType.OK) {
                if (passSlipDAO.updatePassSlipStatus(ps.getSlipId(), "Approved")) {
                    ...
                    printPrompt.setTitle("Print Pass Slip"); printPrompt.setHeaderText("✅  Pass slip approved!");
                    ...
                    printPrompt.showAndWait().ifPresent(choice -> {
                        if (choice == btnPrint) {
                            ...
                            if (pdfPath != null) showAlert(Alert.AlertType.INFORMATION, "PDF Ready", "Pass slip saved to Desktop:\n" + new File(pdfPath).getName() + "\n\nThe PDF has been opened automatically — ready to print!");
                            else showAlert(Alert.AlertType.WARNING, "PDF Failed", "Could not generate PDF.\nMake sure Python + reportlab are installed:\n\n  pip install reportlab");
                        }
                    });
                    loadDashboardData(); loadRecentActivity();
    ...
    private void handleRecordReturn(PassSlip ps) {
        ...
        final String durationOut = (ps.getTimeOut() != null) ? (Duration.between(ps.getTimeOut(), now).toMinutes() / 60) + "h " + (Duration.between(ps.getTimeOut(), now).toMinutes() % 60) + "m" : "—";
        final String lateInfo    = ("Overdue".equalsIgnoreCase(ps.getStatus()) && ps.getTimeIn() != null) ? "\n⚠  Late by      : " + (Duration.between(ps.getTimeIn(), now).toMinutes() / 60) + "h " + (Duration.between(ps.getTimeIn(), now).toMinutes() % 60) + "m" : "";
        Alert confirm = new Alert(Alert.AlertType.CONFIRMATION);
        confirm.setTitle("Confirm Employee Return"); confirm.setHeaderText("Confirm return of " + ps.getEmpName() + "?");
        confirm.setContentText("Pass Slip     :  PS-" + String.format("%04d", ps.getSlipId()) + "\nDepartment    :  " + ps.getDepartment() + "\nTime Out      :  " + ps.getFormattedTimeOut() + "\nExpected In   :  " + ps.getFormattedTimeIn() + "\nActual Return :  " + now.format(DATETIME_FMT) + "\nTotal Duration:  " + durationOut + lateInfo + "\n\nHave you personally verified that\n" + ps.getEmpName() + " has returned to the campus?");
        ...
        confirm.showAndWait().ifPresent(r -> {
            if (r == btnConfirm) {
                if (passSlipDAO.recordTimeIn(ps.getSlipId(), now, durationOut)) {
                    activityLogDAO.logActivity(ps.getEmpId(), "Return confirmed: " + ps.getEmpName() + " (PS-" + ps.getSlipId() + ") at " + now.format(TIME_FMT), currentUser != null ? currentUser.getUsername() : "Admin");
                    showAlert(Alert.AlertType.INFORMATION, "Return Confirmed", "✅  " + ps.getEmpName() + " has been marked as RETURNED.\nActual return  : " + now.format(TIME_FMT) + "\nTotal duration : " + durationOut);
                    cmbFilter.setValue("All"); loadDashboardData(); loadRecentActivity();
                } else showAlert(Alert.AlertType.ERROR, "Error", "Failed to record return. Please try again.");
            }
        });
    }
    ...
    @FXML private void handleLogout() {
        ...
        c.showAndWait().ifPresent(r -> { if (r == ButtonType.OK) {
            stopOverdueChecker();
            try { FXMLLoader loader = new FXMLLoader(getClass().getResource("/main/resources/fxml/Login.fxml")); Parent root = loader.load(); Stage stage = (Stage) lblSidebarUser.getScene().getWindow(); stage.setScene(new Scene(root)); stage.setTitle("Pass Slip Issuance System"); stage.show(); } catch (IOException e) { System.out.println("Logout Error: " + e.getMessage()); }
        }});
    }

    private void navigateTo(String fxmlPath, String title) {
        try { FXMLLoader loader = new FXMLLoader(getClass().getResource(fxmlPath)); Parent root = loader.load(); Stage stage = (Stage) lblSidebarUser.getScene().getWindow(); double w=stage.getWidth(), h=stage.getHeight(); stage.setScene(new Scene(root)); stage.setTitle(title); stage.setWidth(w); stage.setHeight(h); } catch (IOException e) { showAlert(Alert.AlertType.ERROR, "Navigation Error", "Could not open " + title + "."); }
    }
