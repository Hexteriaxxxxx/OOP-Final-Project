...
public class AdminDashboardController implements Initializable {
    ...
    private void setupTableColumns() {
        ...
        colRequestId.setCellValueFactory(
                new PropertyValueFactory<>("slipId")
        );

        colName.setCellValueFactory(
                new PropertyValueFactory<>("empName")
        );
        ...
        colPurpose.setCellValueFactory(
                new PropertyValueFactory<>("reason")
        );

        colTimeOut.setCellValueFactory(
                new PropertyValueFactory<>("formattedTimeOut")
        );

        colTimeIn.setCellValueFactory(
                new PropertyValueFactory<>("formattedTimeIn")
        );
        ...
        colActions.setCellFactory(col -> new TableCell<>() {
            ...
            private final HBox box =
                    new HBox(4, btnView, btnApprove, btnReject);

            {
                ...
                btnApprove.setOnAction(e ->
                        handleApprovePassSlip(
                                getTableView().getItems().get(getIndex())
                        )
                );

                btnReject.setOnAction(e ->
                        handleRejectPassSlip(
                                getTableView().getItems().get(getIndex())
                        )
                );
            }
    ...
    private void loadDashboardData() {
        ...
        try {
            ...
            List<PassSlip> today =
                    passSlipDAO.getTodayPassSlips();

            lblTotalRequests.setText(
                    String.valueOf(today.size())
            );
            ...
            int approvalRate = today.isEmpty()
                    ? 0
                    : (int) ((todayApproved * 100) / today.size());

            lblApprovalRate.setText(
                    approvalRate + "%"
            );

            lblActiveNow.setText(
    ...
    private void loadRecentActivity() {
        ...
        try {
            ...
            List<ActivityLog> logs =
                    activityLogDAO.getRecentLogs(10);

            if (logs.isEmpty()) {

                Label empty =
                        new Label("No recent activity.");
    ...
    private void handleCreatePassSlip() {
        ...
        try {
            ...
            System.out.println(
                    "Open Form Error: " + e.getMessage()
            );

            showAlert(
                    Alert.AlertType.ERROR,
                    "Error",
                    "Could not open Pass Slip form."
            );
        }
    }
    ...
    private void handleApprovePassSlip(PassSlip ps) {
        ...
        confirm.showAndWait().ifPresent(response -> {
            ...
            if (response == ButtonType.OK) {
                ...
                boolean success =
                        passSlipDAO.updatePassSlipStatus(
                                ps.getSlipId(),
                                "Approved"
                        );
        ...
        confirm.showAndWait().ifPresent(response -> {
            ...
            if (response == ButtonType.OK) {
                ...
                try {
                    ...
                    stage.setTitle(
                            "Employee Pass Slip System - Login"
                    );
